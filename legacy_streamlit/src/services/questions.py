...
from src.core.rules import score_dominio, status_aprovacao_bloco
from src.db.models import Block, BlockMastery, QuestionAttempt
from src.services.reviews import upsert_review_from_attempt
...
def register_question_attempt(session: Session, attempt: QuestionAttempt) -> QuestionAttempt:
    session.add(attempt)
    session.flush()
    session.refresh(attempt)

    if attempt.block_id is not None:
        recalculate_block_mastery(session, attempt.block_id)

    upsert_review_from_attempt(session, attempt)
    session.commit()
    session.refresh(attempt)
    return attempt
